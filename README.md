...
`Test` 保留完整代码和测试，是后续开发的集成分支；`master` 只保留生产内容。
所有功能、修复和重构均从 `Test` 创建开发分支，验证后先合并回 `Test`。
发布到 `master` 时，在合并提交前排除 `tests/`、其他测试文件及仅用于测试的配置，解决测试文件冲突时保持生产分支删除。
每次发布均检查排除范围；不将生产分支的测试删除操作回合到 `Test`。
创建提交、合并提交及推送仍按 `AGENTS.md` 逐次取得授权。
