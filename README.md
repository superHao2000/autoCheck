...
`master` 是生产分支，保留签到、配置和通知代码；完整代码、测试以及 `.agents/`、`AGENTS.md` 等开发资料保留在 `Test` 分支。
...
3. 在 `Test` 完成验证后，将生产变更合并到 `master`；提交前移除 `tests/`、`.agents/` 和 `AGENTS.md`，并检查其他新增测试文件及仅用于测试的配置。
4. 检查生产入口和暂存差异后提交。若合并出现测试文件的修改/删除冲突，在 `master` 保持这些文件删除。

测试文件和智能体开发资料的删除只属于生产分支，不向 `Test` 回合该删除操作。每次同步都需要检查排除范围；普通 Git 合并不会自动保证测试文件始终缺席。
